...
import { transferenciaStatusLabels } from "@/lib/constants";
...
export function DashboardTab({ app }: { app: AlmoxarifadoViewModel }) {
 ...
 <section className="app-panel border-2 border-black bg-white text-black p-4 sm:p-5">
 ...
 <section className="app-panel border-2 border-black bg-white text-black">
 ...
 <tr key={produto.id} className="border-t-2 border-black transition-colors hover:bg-neutral-100">
 <td className="px-4 py-3 font-semibold text-black">{produto.nome}</td>
 <td className="px-4 py-3 text-black">{produto.descricao}</td>
 <td className="px-4 py-3 text-right font-bold text-black">{produto.quantidade}</td>
 </tr>
 ))}
 {!app.filteredProdutos.length && (
 <tr>
 <td className="px-4 py-8 text-center text-black" colSpan={3}>
 ...
 >
 <div className="divide-y divide-black divide-y-2">
 ...
 <span className="shrink-0 border-2 border-black bg-neutral-100 px-2 py-1 text-xs font-bold text-black">
 ...
 <p className="py-8 text-center text-sm text-black">Nenhuma transferência cadastrada.</p>
 )}
 </div>
 </DataPanel>
 </div>
