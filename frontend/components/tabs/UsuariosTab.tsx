...
export function UsuariosTab({ app }: { app: AlmoxarifadoViewModel }) {
 ...
 <form onSubmit={app.handleCreateUsuario} className="border-2 border-black bg-white text-black">
 <SectionHeader
 ...
 <p className="py-8 text-center text-sm text-black">Nenhum usuário encontrado.</p>
 )}
 {app.filteredUsuarios.map((usuario) => (
 <article key={usuario.id} className="border-2 border-black bg-white text-black p-4">
 <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
 <div>
 <h3 className="font-bold text-black">{usuario.nome}</h3>
 <p className="mt-1 text-sm text-black">{usuario.email}</p>
 <p className="mt-2 text-xs font-semibold uppercase tracking-[0.12em] text-black">
 {usuario.siglaSecretaria}
 </p>
 </div>
 <div className="flex flex-wrap gap-2">
 <span className="border-2 border-black bg-white px-3 py-1 text-xs font-bold text-black">
 {usuario.roles}
 </span>
 <span className="border-2 border-black bg-white px-3 py-1 text-xs font-bold text-black">
 ...
 className="h-8 border-2 border-black bg-white px-3 text-xs font-bold text-black hover:bg-neutral-100"
 type="button"
 >
 Editar
 </button>
